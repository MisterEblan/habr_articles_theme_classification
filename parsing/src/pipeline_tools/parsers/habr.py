...
from ..data_models import HabrFeed, Page, PageInfo
...
class HabrParser(
    ...
    def parse_with_offset(self, offset: int, n_pages: int = 5) -> List[HabrFeed]:
        ...
        try:
            ...
            for i in range(offset, offset + n_pages):
                url = f"{self.feed_url}/articles/page{i}"
    ...
    def parse_hub_with_offset(
            self,
            hub: str,
            offset: int,
            n_pages: int = 5
    ):
        """Парсит несколько страниц с оффсетом из определённого хаба

        Args:
            hub: название хаба для URL.
            offset: смещение.
            n_pages: количество страниц для парсинга.
        """
        logger.info(
            "Парсим \"%s\". Страниц %s, смещение %s",
            hub, n_pages, offset
        )
        ...
        try:
            feeds = []
            for i in range(offset, offset + n_pages):
                url = f"{self.feed_url}/hubs/{hub}/articles/page{i}"

                logger.info(f"URL: {url}")

                result = requests.get(url)
                logger.info("Код ответа: %s", result.status_code)

                feed: HabrFeed = self._parse_raw_result(result.text)
